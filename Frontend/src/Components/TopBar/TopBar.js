...
const TopBar = ({ darkMode, userName }) => {
    return (
        <div className={`fixed top-0 right-0 p-4 flex items-center ${darkMode ? 'bg-inputBackground text-colorText' : 'bg-gray-300 text-gray-700'}`} style={{ height: '5rem', width: 'calc(100% - 20rem)' }}>
            ...
            <div className="flex items-center">
                <div className="relative mx-8 flex-1 max-w-xs">
                    ...
                    <input 
                        ...
                        className={`h-11 w-96 pl-10 py-2 border rounded-lg outline-none ${darkMode ? 'bg-inputBackground text-colorText border-border' : ''}`}
                    />
                </div>
            </div>

            <div className="flex items-center justify-end bg-purple-600 rounded-full px-3 py-1 ml-auto">
